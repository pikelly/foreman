module ConflictsHelper
  ...
  def dhcp_collision_entries host, collision
    content_tag(:td, dhcp_mac_collision(host, collision), :valign => "top") +
    content_tag(:td, dhcp_ip_collision(host, collision), :valign => "top")
  end
  ...
  def dhcp_ip_collision host, collision
    content_tag(:fieldset) do
      ...
      (collision.dhcp_ip_mac_collision_info ? content_tag(:ul, content_tag(:li, collision.dhcp_ip_mac_collision_info, :style => "color:BurlyWood ;")) : "")
    end
  end

  def legend kind, bool
    legend  = "Found via #{kind.match(/_(.*)/)[1].upcase} address"
