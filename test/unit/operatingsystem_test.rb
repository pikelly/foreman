...
class OperatingsystemTest < ActiveSupport::TestCase
  ...
  test "should be able to access its hosts" do
    ...
    a = Host.create(:name => "fullname1", :ip => "123.123.123.1", :mac => "bbccddeeff11", :operatingsystem => os,
      :domain => domains(:brs), :environment => environments(:production), :ptable => ptables(:default), :architecture => architectures(:x86_64))
    b = Host.create(:name => "fullname2", :ip => "123.123.123.2", :mac => "bbccddeeff22", :operatingsystem => os,
      :domain => domains(:brs), :environment => environments(:production), :ptable => ptables(:default), :architecture => architectures(:x86_64))
  ...
  test "should be able to access its puppetclasses" do
    ...
    assert os.puppetclasses(true).map{|pc| pc.name}.sort == ["apache", "base"]
  end
